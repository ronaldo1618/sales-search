...
searchInput.addEventListener('keypress', event => {
    ...
    if (event.charCode === 13) {
        ...
        const foundAgent = salesByWeek.filter( week => {
            if (week.sales_agent.first_name.includes(searchTerm) || week.sales_agent.last_name.includes(searchTerm) || week.sales_agent.mobile.includes(searchTerm) || week.sales_agent.email.includes(searchTerm)) {
                return true;
            }
        });
        foundAgent.forEach(agent => {
            const name = agent.sales_agent.first_name + " " + agent.sales_agent.last_name
            const email = agent.sales_agent.email
            const phoneNumber = agent.sales_agent.mobile
            container.innerHTML += `<hr><h2>${name}</h2><p>${email}<br>${phoneNumber}</p>`   
        });
    }
})